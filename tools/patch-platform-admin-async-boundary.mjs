...
const workerPath = 'v2/platform-admin-overlay.js';
let source = fs.readFileSync(workerPath, 'utf8');

const replacements = [
  ['return authCheckWithExactAdmin(request, env);', 'return await authCheckWithExactAdmin(request, env);'],
  ['return employerUsage(request, env);', 'return await employerUsage(request, env);'],
  ['return coreWorker.fetch(request, env);', 'return await coreWorker.fetch(request, env);'],
  ['return coreWorker.fetch(request, coreEnvForRequest(request, env));', 'return await coreWorker.fetch(request, coreEnvForRequest(request, env));']
];
...
for (const [before, after] of replacements) {
  ...
  if (matches !== 1) {
    throw new Error(`Expected exactly one async-boundary match for: ${before}; found ${matches}`);
  }
...
`;
fs.writeFileSync('tests/platform-admin-overlay-runtime-audit.mjs', test);

console.log('PLATFORM_ADMIN_ASYNC_BOUNDARY_PATCH=APPLIED');

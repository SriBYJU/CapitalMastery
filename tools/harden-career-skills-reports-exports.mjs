...
function nth(text,from,to,n,label){let start=0,i=-1;for(let k=0;k<n;k++){i=text.indexOf(from,start);if(i<0)throw new Error(`Missing patch target ${n}: ${label}`);start=i+from.length;}return text.slice(0,i)+to+text.slice(i+from.length);}
...
// This is the second Avg. readiness label in the file: the first belongs to the Command Center,
// where reportTrack is already used. The second belongs to the detailed report and uses `professional`.
enterprise=nth(enterprise,"<span>Avg. readiness</span>","<span>${professional?'Avg. readiness':'Avg. measured score'}</span>",2,'employer average label');

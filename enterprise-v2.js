(() => {
  ...
  function roleLabTaskForm(task) {
    ...
    if(task.taskType==='written_decision') {
      ...
      return `<div class="cmv2-email-compose"><div class="cmv2-email-compose-row"><span>To</span><b>${esc(b.from||'Maya Chen — Associate')}</b></div><div class="cmv2-email-compose-row"><span>Subject</span><b>Project Northstar — recommendation & key risks</b></div>${stance?roleLabFieldHtml(stance):''}${text?roleLabFieldHtml(text):''}<div class="cmv2-email-compose-note">Keep it concise: recommendation → valuation evidence → what changed → key risk → next step.</div></div>`;
    }

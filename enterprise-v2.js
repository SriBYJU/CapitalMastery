(() => {
  ...
  function renderV2AssessmentResult(assessment, assignmentId, result) {
    ...
    setMain(`<section class="cmv2-page"><div class="container cmv2-narrow"><div class="card cmv2-result-card"><div class="cmv2-result-score"><strong>${Number(result.correct||0)} / ${Number(result.total||0)}</strong><span>${Number(result.score||0)}% · ${result.passed?'Passed':'Retry required'}</span></div><div><div class="eyebrow">SERVER-GRADED RESULT</div><h1>${result.passed?'Evidence recorded and attempt closed.':'Review and try again.'}</h1><p>${result.passed?'This pass is permanent. Reopening the route will show this attempt question by question, never a blank assessment.':`Your attempt is saved. Review it before retrying; ${Number(result.passScore||assessment.passScore||0)}% is required.`}</p>${r.status?`<div class="cmv2-evidence-note"><b>${esc(readinessLabel(r.status))}</b><span>${Number(r.evidenceCoverage||0)}% professional evidence coverage · ${esc(evidencePhaseLabel(r.evidencePhase))}</span></div>`:''}${issued.length?`<div class="cmv2-issued-v2"><b>New verified credential${issued.length>1?'s':''}</b>${issued.map(c=>`<a href="#/credential/${encodeURIComponent(assessment.pathwayId)}/${encodeURIComponent(c.level)}">${esc(c.title)} →</a>`).join('')}</div>`:''}<div class="cmv2-success-actions"><a class="btn btn-primary" href="${result.passed?next:`#/v2-assessment/${encodeURIComponent(assessment.key)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}`}">${result.passed?(assessment.stage==='final'?'View Readiness Report →':'Continue to Role Lab →'):'Review attempt and retry →'}</a>${assignmentId?`<a class="btn btn-outline" href="#/assigned/${encodeURIComponent(assignmentId)}">Assigned Program</a>`:''}</div></div></div></div></section>`);
  }

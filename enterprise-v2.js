(() => {
  ...
  const STANDARD_STAGES = {
    ...
    career_skills: [
      ...
      { id:'career-capstone', title:'Career Skills Capstone', copy:'Complete the practical job simulation and earn the Career Skills Program Completion Certificate.', required:true },
  ...
  function targetTitle(id) {
    return catalog?.credentialLadder?.find(c => c.id === id)?.title || catalog?.programCompletions?.find(c => c.id === id)?.title || String(id || '').replace(/_/g,' ').replace(/\b\w/g,m=>m.toUpperCase());
  }
  ...
  function lockedStagePreview(key,message,assignmentId=''){
    const final=/professional-final$/i.test(String(key||''));
    const pathway=publicPathId(final?(key==='ib-professional-final'?'investment-banking':String(key).replace(/-professional-final$/i,'')):(key==='ib-essentials-case'?'investment-banking':String(key).replace(/-essentials-case$/i,'')));
    const back=assignmentId?`#/assigned/${encodeURIComponent(assignmentId)}`:`#/career/${encodeURIComponent(pathway)}`;
    setMain(`<section class="cmv2-page cm-course-locked-preview"><div class="container cmv2-narrow"><div class="card cmv2-error"><div class="eyebrow">LOOK AHEAD · READ-ONLY</div><span class="cm-lock-state">🔒 Locked for now</span><h1>${final?'Professional Readiness Final':'Essentials Mini Case'}</h1><p>You can see this stage in the program, but the assessment form and answer controls remain unavailable until its prerequisites are complete.</p><div class="cm-lock-requirements"><strong>Required first</strong><span>${esc(message)}</span><small>Your existing progress and prior attempts are unchanged.</small></div><a class="btn btn-primary" href="${back}">Continue where you left off →</a></div></div></section>`);
  }
  ...
  function employerPublicTourVisual(kind){
    if(kind==='program') return `<div class="cmv2-public-tour-visual"><div class="cmv2-tour-program"><span>Career<b>Private Equity</b></span><i>→</i><span>Track<b>Professional Readiness</b></span><i>→</i><span>Cohort<b>2027 Summer Analysts</b></span><i>→</i><span>Deadline<b>May 30</b></span></div><small>One assignment connects the role, evidence standard, people and timing.</small></div>`;
    if(kind==='work') return `<div class="cmv2-public-tour-visual"><div class="cmv2-tour-work"><div><span>DATA ROOM</span><b>Financials · debt terms · customer evidence</b></div><div><span>WORKBOOK</span><b>Base case · downside · checks · recommendation</b></div><div><span>UPDATE</span><b>New information changes the underwriting</b></div><div><span>MANAGER HANDOFF</span><b>Decision · evidence · risk · next action</b></div></div><small>Learners perform the work in sequence; they do not complete another multiple-choice story.</small></div>`;
    if(kind==='evidence') return `<div class="cmv2-public-tour-visual"><div class="cmv2-tour-evidence"><div><span>Learner</span><b>Analyst 07</b></div><div><span>Completion</span><b>88%</b></div><div><span>Readiness</span><b>81</b></div><div><span>Lowest skill</span><b>Model QA · 68</b></div><div><span>Manager action</span><b>Review revised downside case</b></div></div><small>Completion, readiness, evidence coverage and coaching action stay separate.</small></div>`;
    return `<div class="cmv2-public-tour-visual"><div class="cmv2-tour-govern"><span>OWNER<small>Workspace + people + controls</small></span><span>TRAINING ADMIN<small>Cohorts + assignments + reports</small></span><span>CONTENT MANAGER<small>Firm Layer only</small></span><span>MANAGER<small>Evidence + reviews</small></span><span>VIEWER<small>Read-only reporting</small></span></div><div class="cmv2-guide-callout"><b>Material action recorded</b><p>Role changes, content versions, assignment status, reviews and other controlled actions create audit history.</p></div></div>`;
  }

  function bindEmployerPublicTour(){
    const tabs=[...document.querySelectorAll('[data-employer-tour-tab]')];
    const panels=[...document.querySelectorAll('[data-employer-tour-panel]')];
    tabs.forEach(button=>button.addEventListener('click',()=>{const id=button.dataset.employerTourTab;tabs.forEach(item=>{const active=item===button;item.classList.toggle('active',active);item.setAttribute('aria-selected',String(active));});panels.forEach(panel=>panel.classList.toggle('active',panel.dataset.employerTourPanel===id));}));
  }
  ...
  function employerLanding() {
    ...
    <section class="section cmv2-public-tour"><div class="container"><div class="section-head"><div><div class="eyebrow">INTERACTIVE EMPLOYER WALKTHROUGH</div><h2>Preview the operating model before creating an account.</h2></div><p>Use this guided view to inspect what a training lead configures, what a learner produces, what a manager sees and how access stays controlled.</p></div><div class="cmv2-public-tour-shell"><nav class="card cmv2-public-tour-nav" aria-label="Employer walkthrough steps">${[['program','1 · Configure the program'],['work','2 · Inspect the real work'],['evidence','3 · Read the evidence'],['govern','4 · Govern the rollout']].map((step,index)=>`<button type="button" class="${index===0?'active':''}" data-employer-tour-tab="${step[0]}" aria-selected="${index===0?'true':'false'}">${step[1]}</button>`).join('')}</nav><div>${[['program','Assign the right readiness standard','Choose the exact career, track, cohort and deadline. Capital Mastery keeps credential gates standardized while the Firm Layer remains additive.'],['work','See job work, not course theater','Review the same role-specific workbook, data-room, applied-work and Role Lab formats learners use.'],['evidence','Coach from evidence, not guesswork','Open a learner drilldown and distinguish completion, readiness, evidence coverage, revision history and the next manager action.'],['govern','Keep the rollout reviewable','Separate organization roles, protect the Capital Mastery Standard, version Firm Layer content and retain audit history.']].map((step,index)=>`<article class="card cmv2-public-tour-panel ${index===0?'active':''}" data-employer-tour-panel="${step[0]}"><div class="eyebrow">${step[1].toUpperCase()}</div><h3>${step[1]}</h3><p>${step[2]}</p>${employerPublicTourVisual(step[0])}</article>`).join('')}</div></div><div class="cmv2-public-tour-cta"><div><b>Inside a real workspace, the six-step Launch Guide links each explanation directly to the live control.</b><span>No sales call or demo tenant is required to learn the product.</span></div><a class="btn btn-primary" href="#/employer-start">Open Free Employer Workspace →</a></div></div></section>
  ...
  async function submitQuickAssign(event, orgId) {
    ...
    try {
      ...
      const c=await api(`/enterprise/organizations/${encodeURIComponent(orgId)}/cohorts`,{method:'POST',body:JSON.stringify({name:String(fd.get('cohortName')),pathwayId,programLevel:track==='professional'?'professional':'career_skills',deadlineAt:dueAt})});
      status.textContent='Assigning readiness program…';
      const a=await api(`/enterprise/organizations/${encodeURIComponent(orgId)}/assignments`,{method:'POST',body:JSON.stringify({cohortId:c.cohort.id,pathwayId,track,credentialTarget:track==='professional'?'professional_readiness':'career',dueAt})});
      await Promise.all([
  ...
  function assignedProgressSummary(report={},track='professional') {
    ...
    const gates=[levels.has('foundations'),!!report.diagnostic,levels.has('essentials'),levels.has('applied'),levels.has('role_lab')||report.roleLab?.status==='completed',!!report.finalAssessment?.passed];
    const complete=gates.filter(Boolean).length,total=gates.length;
  ...
  async function assignedDetail(id) {
    ...
    try {
      ...
      const report=await api(`/enterprise/learner/readiness-report/${encodeURIComponent(a.pathwayId)}?assignmentId=${encodeURIComponent(a.id)}`);
  ...
  function renderDiagnosticResult(pathwayId, assignmentId, result) {
    ...
    setMain(`<section class="cmv2-page"><div class="container cmv2-narrow"><div class="card cmv2-result-card"><div class="cmv2-result-score"><strong>${Number(result.score||0)}</strong><span>Baseline diagnostic</span></div><div><div class="eyebrow">DIAGNOSTIC COMPLETE</div><h1>${result.enrichmentPending?'Baseline saved. Readiness is still updating.':'Your starting point is recorded.'}</h1>${result.enrichmentPending?`<div class="cmv2-warning-soft" data-cm-diagnostic-pending><strong>Your score is saved.</strong><br>${esc(result.enrichmentWarning||'Readiness calculations are still updating. Do not resubmit the diagnostic.')}</div>`:''}<p>You answered ${Number(result.correct||0)} of ${Number(result.total||0)} questions correctly. This score has <b>0% credential weight</b>.</p><div class="cmv2-evidence-note"><b>${esc(readinessLabel(r.status))}</b><span>${Number(r.evidenceCoverage||0)}% professional evidence coverage · ${esc(evidencePhaseLabel(r.evidencePhase))}</span></div><div class="cmv2-success-actions"><a class="btn btn-primary" href="#/skills/${encodeURIComponent(pathwayId)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}">View Skill Profile →</a><a class="btn btn-outline" href="#/v2-assessment/${encodeURIComponent(assessmentKey(pathwayId,'essentials'))}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}">Continue to Essentials →</a></div></div></div></div></section>`);
  }
  ...
  function v2AssessmentReviewHtml(review) {
    ...
    return `<section class="cmv2-saved-review" aria-label="Saved assessment review"><div class="card cmv2-review-summary"><div class="cmv2-result-score"><strong>${Number(review.correct||0)} / ${Number(review.total||questions.length)}</strong><span>${Number(review.score||0)}% · ${review.passed?'Passed':'Retry required'}</span></div><div><div class="eyebrow">SAVED SERVER-GRADED ATTEMPT · READ ONLY</div><h2>Your submitted work and feedback</h2><p>${review.passed?'This passed attempt is final and cannot be started again.':'Review every answer below, then start a new attempt when you are ready.'} Submitted ${esc(fmtDate(review.submittedAt))}.</p></div></div><div class="cmv2-review-list">${questions.map((q,i)=>`<article class="card cmv2-review-question ${q.correct?'correct':'incorrect'}"><header><span>${String(Number(q.position||i+1)).padStart(2,'0')}</span><b>${q.correct?'Correct':'Review needed'}</b></header><h3>${esc(q.prompt||`Question ${i+1}`)}</h3><dl><div><dt>Your answer</dt><dd>${esc(q.submitted===''?'No answer submitted':q.submitted)}</dd></div><div><dt>Correct answer</dt><dd>${esc(q.correctAnswer)}</dd></div></dl>${q.rationale?`<p><b>Why:</b> ${esc(q.rationale)}</p>`:''}</article>`).join('')}</div></section>`;
  }
  ...
  async function v2AssessmentPage(key, assignmentId='') {
    ...
    try {
      let savedReview=await loadV2SavedReview(key,assignmentId);
      ...
      if(!savedReview&&d.review) savedReview=d.review;
      const back=assignmentId?`#/assigned/${encodeURIComponent(assignmentId)}`:`#/career/${encodeURIComponent(publicPathId(a.pathwayId))}`;
      if(savedReview) {
        setMain(`<section class="cmv2-page"><div class="container cmv2-narrow-wide"><a class="cmv2-back" href="${back}">← Back</a><div class="cmv2-page-head"><div><div class="eyebrow">${esc(String(a.stage||'assessment').replace(/_/g,' ').toUpperCase())} · SERVER GRADED</div><h1>${esc(a.title)}</h1><p>Your saved attempt is read from the authoritative learner record.</p></div>${savedReview.passed?'':`<button class="btn btn-primary" id="cmv2-retake-assessment" type="button">Retry required · Start new attempt</button>`}</div>${v2AssessmentReviewHtml(savedReview)}</div></section>`);
        if(!savedReview.passed) document.getElementById('cmv2-retake-assessment')?.addEventListener('click',()=>renderV2AssessmentForm(a,questions,assignmentId,back));
        return;
      }
      renderV2AssessmentForm(a,questions,assignmentId,back);
    } catch(e){
      if([403,409].includes(Number(e.status))||/complete|earn|prerequisite/i.test(String(e.message||''))) lockedStagePreview(key,e.message,assignmentId);
      else errorPage('Assessment unavailable.',e.message,assignmentId?`#/assigned/${assignmentId}`:'#/careers');
    }
  }
  ...
  function renderV2AssessmentResult(assessment, assignmentId, result) {
    ...
    setMain(`<section class="cmv2-page"><div class="container cmv2-narrow"><div class="card cmv2-result-card"><div class="cmv2-result-score"><strong>${Number(result.correct||0)} / ${Number(result.total||0)}</strong><span>${Number(result.score||0)}% · ${result.passed?'Passed':'Retry required'}</span></div><div><div class="eyebrow">SERVER-GRADED RESULT</div><h1>${result.evidenceRefreshPending||result.credentialRefreshPending?'Assessment saved. Post-processing is still updating.':result.passed?'Evidence recorded and attempt closed.':'Review and try again.'}</h1>${result.evidenceRefreshPending||result.credentialRefreshPending?`<div class="cmv2-warning-soft" data-cm-assessment-pending><strong>Your assessment score is permanently saved.</strong><br>${esc(result.evidenceRefreshWarning||result.credentialRefreshWarning||'Readiness or credential processing is still updating. Do not resubmit the assessment.')}</div>`:''}<p>${result.passed?'This pass is permanent. Reopening the route will show this attempt question by question, never a blank assessment.':`Your attempt is saved. Review it before retrying; ${Number(result.passScore||assessment.passScore||0)}% is required.`}</p>${r.status?`<div class="cmv2-evidence-note"><b>${esc(readinessLabel(r.status))}</b><span>${Number(r.evidenceCoverage||0)}% professional evidence coverage · ${esc(evidencePhaseLabel(r.evidencePhase))}</span></div>`:''}${issued.length?`<div class="cmv2-issued-v2"><b>New verified credential${issued.length>1?'s':''}</b>${issued.map(c=>`<a href="#/credential/${encodeURIComponent(assessment.pathwayId)}/${encodeURIComponent(c.level)}">${esc(c.title)} →</a>`).join('')}</div>`:''}<div class="cmv2-success-actions"><a class="btn btn-primary" href="${result.passed?next:`#/v2-assessment/${encodeURIComponent(assessment.key)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}`}">${result.passed?(assessment.stage==='final'?'View Readiness Report →':'Continue to Role Lab →'):'Review attempt and retry →'}</a>${assignmentId?`<a class="btn btn-outline" href="#/assigned/${encodeURIComponent(assignmentId)}">Assigned Program</a>`:''}</div></div></div></div></section>`);
  }
  ...
  async function learnerReadinessReport(pathwayId, assignmentId='') {
    ...
    try {
      ...
      setMain(`<section class="cmv2-page"><div class="container"><a class="cmv2-back" href="${assignmentId?`#/assigned/${encodeURIComponent(assignmentId)}`:`#/career/${encodeURIComponent(publicPathId(pathwayId))}`}">← Back</a><div class="cmv2-report-hero"><div><div class="eyebrow">${professional?'VERIFIED READINESS REPORT':'VERIFIED CAREER SKILLS REPORT'}</div><h1>${esc(d.pathway.title)}</h1><p>${esc(d.pathway.role)} · Generated ${fmtDate(d.generatedAt)}</p></div><div class="cmv2-report-score"><strong>${r?Number(r.overallScore):'—'}</strong><span>${esc(professional?readinessLabel(r?.status):(careerVerified?'Career Skills verified':'Career Skills evidence'))}</span><small>${r?Number(r.evidenceCoverage):0}% ${professional?'professional':'Career Skills'} evidence coverage</small></div></div><div class="cmv2-kpis"><div class="card"><strong>${professional?(d.diagnostic?Number(d.diagnostic.score):'—'):'N/A'}</strong><span>${professional?'Baseline':'Baseline not required'}</span></div><div class="card"><strong>${r?Number(r.overallScore):'—'}</strong><span>${professional?'Current readiness':'Measured skill score'}</span></div><div class="card"><strong>${professional?(r&&r.improvement!=null?(Number(r.improvement)>=0?'+':'')+Number(r.improvement):'—'):(careerVerified?'✓':'—')}</strong><span>${professional?'Improvement':'Career Skills credential'}</span></div><div class="card"><strong>${professional?(d.roleLab?.score??'—'):'N/A'}</strong><span>${professional?'Role Lab':'Role Lab not required'}</span></div></div><section class="card"><div class="cmv2-card-head"><div><div class="eyebrow">COMPETENCY EVIDENCE</div><h2>What the readiness score represents</h2></div><a href="#/skills/${encodeURIComponent(pathwayId)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}">Detailed profile →</a></div><div class="cmv2-report-competencies">${skills.map(x=>`<div><span>${esc(x.name)}${x.critical?' · Critical':''}</span><div class="cmv2-report-bar"><i style="width:${Math.max(0,Math.min(100,Number(x.score)))}%"></i><b style="left:${Math.max(0,Math.min(100,Number(x.minimumScore)))}%"></b></div><strong>${Number(x.score)}</strong><small>${Number(x.evidenceCount)} evidence records</small></div>`).join('')||'<p>No competency evidence yet.</p>'}</div></section><div class="cmv2-two-col"><section class="card"><div class="eyebrow">WORK EVIDENCE</div><h2>Applied proof</h2><div class="cmv2-evidence-list"><div><span>Diagnostic</span><b>${professional?(d.diagnostic?`${Number(d.diagnostic.score)}%`:'Not completed'):'Not required'}</b></div><div><span>Role Lab</span><b>${professional?(d.roleLab?`${Number(d.roleLab.score||0)}% · ${esc(d.roleLab.status)}`:'Not completed'):'Not required'}</b></div><div><span>Professional Final</span><b>${professional?(d.finalAssessment?`${Number(d.finalAssessment.score)}% · ${d.finalAssessment.passed?'Passed':'Not passed'}`:'Not completed'):'Not required'}</b></div></div></section><section class="card"><div class="eyebrow">VERIFIED CREDENTIALS</div><h2>Stackable achievements</h2><div class="cmv2-evidence-list">${creds.map(c=>`<a href="#/credential/${encodeURIComponent(pathwayId)}/${encodeURIComponent(c.level)}"><span>${esc(c.title)}</span><b>${esc(c.status)}</b></a>`).join('')||'<p>No credentials yet.</p>'}</div></section></div></div></section>`);
  ...
  function downloadReportJson(report) {
    ...
    const exportPayload={
      ...
      programScope:{
        ...
        completionRecord:professional?'professional_readiness':'career',
  ...
  async function roleLabLanding(pathwayId, assignmentId='') {
    ...
    try {
      await loadCatalog(); const d=await api(`/enterprise/role-labs/${encodeURIComponent(pathwayId)}${assignmentId?`?assignmentId=${encodeURIComponent(assignmentId)}`:''}`); const lab=(d.labs||[])[0]; if(!lab) throw new Error('A professional Role Lab for this pathway is not live yet.'); const s=lab.scenario||{}; const access=d.access||{status:'available',missing:[],reason:'Prerequisites complete.'};
      ...
      const finalKey=assessmentKey(pathwayId,'final');
      const action=access.status==='passed'?`<a class="btn btn-primary btn-block" href="#/v2-assessment/${encodeURIComponent(finalKey)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}">Continue to Professional Final →</a>`:access.status==='locked'?`<span class="btn btn-soft btn-block cm-disabled-action" aria-disabled="true">Complete prerequisites to unlock</span>`:`<button class="btn btn-primary btn-block" id="cmv2-start-lab">Start / Resume Role Lab →</button>`;
      setMain(`<section class="cmv2-page"><div class="container"><a class="cmv2-back" href="${assignmentId?`#/assigned/${encodeURIComponent(assignmentId)}`:`#/career/${encodeURIComponent(publicPathId(pathwayId))}`}">← Back</a><div class="cmv2-rolelab-hero"><div><div class="eyebrow">ROLE LAB · ${esc(lab.roleTitle)}</div><h1>${esc(lab.title)}</h1><p>${esc(s.context||'')}</p><div class="cmv2-rolelab-meta"><span><b>Desk</b>${esc(s.desk||pathTitle(pathwayId))}</span><span><b>Reviewer</b>${esc(s.reviewer||s.associate||'Manager')}</span><span><b>Client / project</b>${esc(lab.clientName||s.client||s.buyer||'Synthetic case')}</span><span><b>Pass standard</b>${Number(lab.passScore||80)}%</span></div></div><div class="cmv2-rolelab-badge">LIVE-STYLE<br>WORKFLOW</div></div><div class="cmv2-two-col"><section class="card"><div class="eyebrow">WHAT YOU WILL DO · READ-ONLY PREVIEW</div><h2>Perform the work—not answer trivia.</h2><div class="cmv2-work-list">${workflow.map((x,i)=>`<div><b>${String(i+1).padStart(2,'0')}</b><span>${esc(x)}</span></div>`).join('')}</div></section><section class="card cmv2-start-lab"><div class="eyebrow">${access.status==='passed'?'ROLE LAB PASSED':access.status==='locked'?'LOCKED FOR NOW':'PROJECT FILE READY'}</div><h2>${esc(s.project||s.target||lab.clientName||lab.title)}</h2><p>${s.deadline?`<b>Deadline:</b> ${esc(s.deadline)}<br>`:''}The source files and work instructions are provided inside the secure case. Your outputs and revisions become competency evidence.</p><div class="cmv2-warning-soft"><strong>${esc(access.reason)}</strong><br>All companies, people and data are synthetic training materials. No proprietary employer information is used.</div>${action}<a class="btn btn-outline btn-block" href="#/skills/${encodeURIComponent(pathwayId)}${assignmentId?`?assignment=${encodeURIComponent(assignmentId)}`:''}">View Skill Profile</a><div class="cmv2-form-status"></div></section></div></div></section>`);
  ...
  function adminIntegrityCard(result={}) {
    const quick=Array.isArray(result.quickCheck)?result.quickCheck:[];
    const quickOk=quick.some(row=>Object.values(row||{}).some(value=>String(value).toLowerCase()==='ok'));
    const violations=Array.isArray(result.foreignKeyViolations)?result.foreignKeyViolations:[];
    const tableCount=result.tableCounts&&typeof result.tableCounts==='object'&&!Array.isArray(result.tableCounts)?Object.keys(result.tableCounts).length:0;
    const healthy=result.ok===true&&quickOk&&violations.length===0&&tableCount>0;
    return `<section class="card cmv2-admin-integrity" data-cm-admin-integrity="${healthy?'pass':'fail'}"><div class="cmv2-card-head"><div><div class="eyebrow">PRODUCTION DATA HEALTH</div><h2>${healthy?'D1 integrity verified':'Integrity check needs attention'}</h2><p>${healthy?`SQLite quick check passed, no foreign-key violations were found, and ${tableCount} production tables were inspected.`:esc(result.error||'The integrity endpoint did not return a complete healthy result. Review the release before using real employer data.')}</p></div><span class="cmv2-status ${healthy?'ready':'attention'}">${healthy?'VERIFIED':'ATTENTION'}</span></div><div class="cmv2-warning-soft"><strong>Release rule:</strong> this read-only check never changes employer or learner records. Synthetic lifecycle tests must still be reset after each release.</div></section>`;
  }
  ...
  async function adminDemoLab() {
    ...
    try {
      ...
      await loadCatalog();
      const [state,perm,integrity]=await Promise.all([
        api('/enterprise/admin/demo'),
        api('/enterprise/admin/demo/permission-matrix'),
        api('/admin/integrity').catch(error=>({ok:false,error:error.message}))
      ]);
      const demos=state.demos||[];
      const permissionRoles=perm.roles||{};
      ...
      document.querySelector('.cmv2-demo-existing')?.insertAdjacentHTML('beforebegin',adminIntegrityCard(integrity));

(() => {
  ...
  function employerLanding() {
    ...
    setMain(`<section class="cmv2-employer-hero"><div class="container cmv2-hero-grid"><div><div class="eyebrow">CAPITAL MASTERY FOR EMPLOYERS</div><h1>Make new finance talent productive sooner.</h1><p>Role-specific finance preparation, realistic work simulations, competency measurement and verified evidence—before Day 1.</p><div class="hero-actions"><a class="btn btn-primary" href="#/employer-start">Get Started / Open Employer Workspace →</a><a class="btn btn-outline" href="#/careers">Preview Career Training</a></div><div class="cmv2-trust-row"><span>✓ Standardized readiness</span><span>✓ Firm customization</span><span>✓ Evidence-backed coaching</span></div></div><div class="cmv2-preview-panel"><div class="cmv2-preview-top"><span>2027 Summer Analysts</span><b>Readiness 87</b></div><div class="cmv2-meter"><i style="width:87%"></i></div><div class="cmv2-mini-grid"><div><strong>92</strong><span>Accounting</span></div><div><strong>89</strong><span>Valuation</span></div><div><strong>81</strong><span>Modeling</span></div><div><strong>90</strong><span>Communication</span></div></div><div class="cmv2-alert">2 analysts need additional model-review practice.</div></div></div></section>
  ...
  function employerGuideSteps(orgId, org, cohorts, assignments, report) {
    const learners=report?.learners||[];
    return [
      {id:'workspace',title:'Understand the Command Center',copy:'See what Capital Mastery measures, where manager attention appears, and what the readiness score does—and does not—mean.',done:true,href:`#/employer/${orgId}`,visual:'command'},
      {id:'assign',title:'Create a cohort & assign a role',copy:'Choose the career, readiness level, learners and deadline. Capital Mastery protects the standardized core underneath.',done:assignments.length>0,href:`#/employer/${orgId}/quick-assign`,visual:'assign'},
      ...
      {id:'govern',title:'Know the controls before rollout',copy:'Roles, Firm Layer changes and material actions are server-enforced and audited. Review Team & Roles and the Audit Log before a real cohort launch.',done:false,href:`#/employer/${orgId}/team`,visual:'govern'}
    ];
  }
  ...
  async function employerGuidePage(orgId) {
    ...
    try {
      ...
      const ack=guideAcknowledged(orgId); const steps=employerGuideSteps(orgId,orgData.organization,cohorts,assignments,report); const understood=steps.filter(x=>ack.has(x.id)).length;
  ...
  async function orgDashboard(orgId) {
    ...
    try {
      ...
      const summary=dash.summary || {}; const cs=cohorts.cohorts||[]; const as=(assignments.assignments||[]).filter(x=>x.status!=='archived');
      let report=null;
      if(as[0]) { try { const r=await api(`/enterprise/organizations/${encodeURIComponent(orgId)}/readiness-report?assignmentId=${encodeURIComponent(as[0].id)}`); report=r.assignments?.[0]||null; } catch {} }
      ...
      const launchSteps=employerGuideSteps(orgId,org.organization,cs,as,report); const launchDone=launchSteps.filter(x=>x.done).length;
      setMain(`<section class="cmv2-page"><div class="container"><a class="cmv2-back" href="#/employer">← Employer workspaces</a><div class="cmv2-page-head cmv2-org-head"><div><div class="eyebrow">EMPLOYER COMMAND CENTER</div><h1>${esc(org.organization.name)}</h1><p>${esc(org.membership.role.replace(/_/g,' '))} access · Capital Mastery Enterprise</p></div><div class="cmv2-head-actions"><a class="btn btn-gold" href="#/employer/${encodeURIComponent(orgId)}/guide">Interactive Launch Guide</a><a class="btn btn-outline" href="#/notifications">Notifications</a><a class="btn btn-primary" href="#/employer/${encodeURIComponent(orgId)}/quick-assign">Quick Assign →</a><a class="btn btn-outline" href="#/employer/${encodeURIComponent(orgId)}/curriculum">Curriculum & Firm Layer</a><a class="btn btn-outline" href="#/employer/${encodeURIComponent(orgId)}/reports">Readiness Reports</a>${['owner','training_admin'].includes(org.membership.role)?`<a class="btn btn-outline" href="#/employer/${encodeURIComponent(orgId)}/team">Team & Roles</a><a class="btn btn-outline" href="#/employer/${encodeURIComponent(orgId)}/audit">Audit Log</a>`:''}</div></div><section class="card cmv2-launch-strip"><div><div class="eyebrow">LAUNCH READINESS</div><h2>${launchDone}/${launchSteps.length} workspace signals ready</h2><p>Use the interactive guide before a real firm rollout. It explains what each employer view means and links directly into the live workflow.</p></div><div class="cmv2-launch-progress"><i><b style="width:${Math.round(launchDone/launchSteps.length*100)}%"></b></i><a href="#/employer/${encodeURIComponent(orgId)}/guide">Continue guided setup →</a></div></section><div class="cmv2-kpis cmv2-kpis-6"><div class="card"><strong>${Number(summary.learners||0)}</strong><span>Active learners</span></div><div class="card"><strong>${startedCount||0}</strong><span>Started</span></div><div class="card"><strong>${readyCount||0}</strong><span>Ready / complete</span></div><div class="card"><strong>${summary.averageReadiness == null ? '—' : esc(summary.averageReadiness)}</strong><span>Avg. readiness</span></div><div class="card"><strong>${avgEvidence==null?'—':avgEvidence+'%'}</strong><span>Avg. evidence coverage</span></div><div class="card"><strong>${attention.length}</strong><span>Need attention</span></div></div><div class="cmv2-two-col"><section class="card"><div class="cmv2-card-head"><div><div class="eyebrow">COHORTS</div><h2>Training groups</h2></div><a href="#/employer/${encodeURIComponent(orgId)}/quick-assign">+ New</a></div>${cs.length?`<div class="cmv2-list">${cs.map(c=>`<div class="cmv2-list-row"><div><b>${esc(c.name)}</b><span>${esc(pathTitle(c.pathway_id))} · ${esc(c.program_level)}</span></div><div><span class="cmv2-status ${esc(c.status)}">${esc(c.status)}</span><small>${fmtDate(c.deadline_at)}</small></div></div>`).join('')}</div>`:`<div class="cmv2-empty-inline">No cohorts yet. Quick Assign creates the cohort and program together.</div>`}</section><section class="card"><div class="cmv2-card-head"><div><div class="eyebrow">ASSIGNMENTS</div><h2>Active programs</h2></div></div>${as.length?`<div class="cmv2-list">${as.map(a=>`<a class="cmv2-list-row" href="#/employer/${encodeURIComponent(orgId)}/curriculum?assignment=${encodeURIComponent(a.id)}"><div><b>${esc(pathTitle(a.pathway_id))}</b><span>${esc(targetTitle(a.credential_target))}</span></div><div><span class="cmv2-status ${esc(a.status)}">${esc(a.status)}</span><small>${fmtDate(a.due_at)}</small></div></a>`).join('')}</div>`:`<div class="cmv2-empty-inline">No programs assigned yet.</div>`}</section></div><section class="card cmv2-attention-card"><div class="cmv2-card-head"><div><div class="eyebrow">MANAGER ATTENTION QUEUE</div><h2>Spend manager time where it changes the outcome.</h2><p>Prioritized from deadline, revision and evidence signals—not arbitrary engagement scores.</p></div><a href="#/employer/${encodeURIComponent(orgId)}/reports${as[0]?`?assignment=${encodeURIComponent(as[0].id)}`:''}">Full cohort report →</a></div>${attention.length?`<div class="cmv2-attention-list">${attention.map(({learner:x,signal:g},i)=>`<article><span class="cmv2-attention-rank">${i+1}</span><div><b>${esc(x.name||x.email||'Learner')}</b><small>${esc(learnerProgressStage(x).label)}</small><p>${esc(g.reason)}</p></div><div><strong>${esc(g.label)}</strong><span>${esc(g.action)}</span></div></article>`).join('')}</div>`:`<div class="cmv2-empty-inline"><b>No urgent coaching signals right now.</b><p>${learners.length?'The current cohort has no overdue/revision/readiness alerts from the available evidence.':'Signals appear once learners begin producing evidence.'}</p></div>`}</section></div></section>`);
  ...
  async function curriculum(orgId, selectedAssignment='') {
    ...
    try {
      ...
      document.querySelectorAll('[data-content-edit]').forEach(b=>b.addEventListener('click',()=>{const item=content.find(x=>x.id===b.dataset.contentEdit);if(item)renderEditContent(orgId,chosen,item);}));
      document.querySelectorAll('[data-content-history]').forEach(b=>b.addEventListener('click',async()=>{b.disabled=true;try{const h=await api(`/enterprise/organizations/${encodeURIComponent(orgId)}/firm-content/${encodeURIComponent(b.dataset.contentHistory)}/versions`);renderContentHistory(orgId,chosen,h);}catch(err){alert(err.message);b.disabled=false;}}));
  ...
  async function employerReadinessReport(orgId, selectedAssignment='') {
    ...
    try {
      await loadCatalog(); const [as,orgData]=await Promise.all([api(`/enterprise/organizations/${encodeURIComponent(orgId)}/assignments`),api(`/enterprise/organizations/${encodeURIComponent(orgId)}`)]); const assignments=(as.assignments||[]).filter(x=>x.status!=='archived'); const chosen=assignments.find(x=>x.id===selectedAssignment)||assignments[0]; const canReview=['owner','training_admin','manager'].includes(orgData.membership?.role);

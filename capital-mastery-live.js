(() => {
  ...
  function renderCareerSimulationWorkbench(data,pathwayId,itemId,el){
    ...
    el.innerHTML=`<section class="cm-wb-shell">
      <div class="cm-wb-top"><div class="container cm-wb-top-inner"><div><div class="eyebrow">PROFESSIONAL WORKBENCH · SYNTHETIC CASE</div><h1>${esc(p.project)}</h1><p>${esc(p.role)}</p></div><div class="cm-wb-deadline"><span>Review deadline</span><strong>${esc(p.deadline)}</strong></div></div></div>
      <div class="container cm-wb-layout">
        ...
        <main class="cm-wb-main">
          <section class="cm-wb-hero"><div><div class="eyebrow">ASSIGNMENT</div><h2>${esc(p.objective)}</h2><p>Use the source files, produce the requested work outputs, then send a structured reviewer-facing recommendation. The separate final checks knowledge, calculations and workflow judgment; this screen tests whether you can do the job itself.</p></div><div class="cm-wb-status"><b>${data.masteryScore}%</b><span>minimum standard</span><small>${esc(data.assessmentVersion)}</small></div></section>

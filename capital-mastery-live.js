(() => {
  ...
  async function renderVerify(publicToken) {
    ...
    const el = main();
    try {
      const data = await apiFetch(`/verify/${encodeURIComponent(publicToken)}`, {}, false);
      const c = data.credential;
      el.innerHTML = `<section class="section"><div class="container" style="max-width:880px"><div class="cm-verification ${data.valid ? 'valid' : 'invalid'}"><span class="cm-verify-badge">${data.valid ? 'VERIFIED ACTIVE CREDENTIAL ✓' : 'NOT ACTIVE'}</span><div class="eyebrow">CAPITAL MASTERY CREDENTIAL</div><h1 class="serif">${esc(c.title)}</h1><p>Issued to <strong>${esc(c.holderName)}</strong></p><div class="grid grid-2"><div class="card"><strong>Credential ID</strong><p>${esc(c.credentialId)}</p></div><div class="card"><strong>Issued</strong><p>${formatDate(c.issuedAt)}</p></div><div class="card"><strong>Level</strong><p>${esc(c.level)}</p></div><div class="card"><strong>Status</strong><p>${esc(c.status)}</p></div></div><div class="cm-security-note"><strong>Verification source:</strong> Capital Mastery secure API → Cloudflare Worker → D1 authoritative credential record.</div></div></div></section>`;
    } catch (error) {

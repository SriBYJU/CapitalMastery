(() => {
    ...
    return `
      <section class="cm-public-certificate-section" aria-label="Capital Mastery certificate">
        <div class="cm-public-cert-heading">
          ...
          <button class="btn btn-outline cm-public-cert-print" type="button">Print / Save Certificate</button>
        </div>

        <div class="cm-public-cert-frame ${valid ? '' : 'not-active'}">
          <div class="certificate ${info.className}">
            ...
            <div class="cert-inner">
              ...
              <div class="cert-bottom">
                <div class="cert-meta">Issued<strong>${esc(formatDate(c.issuedAt))}</strong></div>
                ...
                <div class="cert-meta">Credential ID<strong>${esc(c.credentialId)}</strong></div>
              </div>
            </div>
  ...
  async function enhanceVerifyPage() {
    ...
    try {
      ...
      document.querySelector('.cm-public-cert-print')?.addEventListener('click', () => window.print());
  ...
  style.textContent = `
    ...
    @media(max-width:700px){.cm-public-cert-heading{display:grid;align-items:start}.cm-public-cert-print{width:100%}.cm-public-cert-frame{padding:9px;border-radius:14px}.cm-public-certificate-section{margin-top:20px}.cm-public-cert-heading h2{font-size:1.65rem}}
    @media print{body *{visibility:hidden!important}.cm-public-certificate-section,.cm-public-certificate-section *{visibility:visible!important}.cm-public-certificate-section{position:absolute;left:0;top:0;width:100%;margin:0}.cm-public-cert-heading,.cm-public-cert-proof{display:none!important}.cm-public-cert-frame{border:0;padding:0;background:#fff}.cm-public-cert-frame .certificate{box-shadow:none!important;width:100%!important;max-width:none!important}.site-header,.footer,#cm-madeline-root{display:none!important}}
  `;

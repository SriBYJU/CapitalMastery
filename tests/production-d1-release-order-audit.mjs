...
must(tool.includes("'migrations/016_phase2_career_skills_track_constraints.sql'"),'D1 tool may conditionally apply exact migration 016');
must(tool.includes("'migrations/017_phase2_program_completion_records.sql'"),'D1 tool may conditionally apply exact migration 017');
...
console.log('PRODUCTION D1 RELEASE ORDER AUDIT PASS: source/browser -> schema-aware 016/017 -> integrity -> Worker -> Pages ordering is fail-closed');
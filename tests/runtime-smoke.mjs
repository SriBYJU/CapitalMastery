...
vm.createContext(ctx);vm.runInContext(data,ctx);windowObj.CM_DATA=ctx.window.CM_DATA;vm.runInContext(appCode,ctx);
localStorage.setItem('capitalMasteryQaPreviewV1','true');
const ids=ctx.window.CM_DATA.careers.map(c=>c.id);
const routes=['','careers','credentials','passport','compare','about','methodology','login','privacy','terms','disclaimer','credential-policy','admin-preview'];

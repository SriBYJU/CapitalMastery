...
(async()=>{
  ...
  try{
    ...
    await context.route(`${WORKER}/**`,route=>{
      ...
      if(url.pathname==='/enterprise/assessments/ib-essentials-case/review') return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,review:{attemptId:'v2a_saved',assessmentKey:'ib-essentials-case',pathwayId:'investment-banking',score:75,passed:true,correct:1,total:2,submittedAt:'2026-08-30T12:00:00Z',questions:[{id:'q1',position:1,type:'numeric',prompt:'Calculate enterprise value.',submitted:'850',correct:true,correctAnswer:'850',rationale:'Equity value plus debt less cash.'},{id:'q2',position:2,type:'mc',prompt:'Choose the defensible peer.',submitted:'Highest multiple',correct:false,correctAnswer:'Similar business model',rationale:'Business-model comparability is the defensible basis.'}]}})});
    ...
    assert(/Highest multiple/.test(text)&&/Similar business model/.test(text),'Incorrect submitted answer and correction must both be visible after submission');
    ...
    assert(await page.locator('.cmv2-saved-review').count()===0,'Explicit retake did not replace read-only review');
    ...
    assert(submits===1,'Explicit retake should create exactly one new server submission');
    console.log('V2 ASSESSMENT REVIEW BROWSER AUDIT PASS: authoritative saved answers, corrections, rationale, read-only default, and explicit retake verified');

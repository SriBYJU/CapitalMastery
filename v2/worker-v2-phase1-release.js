    ...
    try {
      ...
      if (request.method === 'GET' && parts[0] === 'enterprise' && parts[1] === 'organizations' && parts[3] === 'readiness-report' && parts.length === 4) {
        ...
        for(const a of assignments){
          ...
          reportAssignments.push({assignment:{id:a.id,cohortId:a.cohort_id,cohortName:a.cohort_name,pathwayId:a.pathway_id,track:a.track,credentialTarget:a.credential_target,status:a.status,dueAt:a.due_at,curriculumVersion:a.curriculum_version},summary:{learners:learnerRows.length,measured:readinessMeasured.length,completed:learnerRows.filter(x=>x.complete).length,overdue:learnerRows.filter(x=>x.overdue).length,averageReadiness:readinessMeasured.length?Math.round(readinessMeasured.reduce((s,x)=>s+Number(x.readiness.overallScore||0),0)/readinessMeasured.length):null,averageImprovement:readinessMeasured.filter(x=>x.readiness.improvement!=null).length?Math.round(readinessMeasured.filter(x=>x.readiness.improvement!=null).reduce((s,x)=>s+Number(x.readiness.improvement||0),0)/readinessMeasured.filter(x=>x.readiness.improvement!=null).length):null},competencies:competencySummary,learners:learnerRows});
        }
      ...
      if (request.method === 'GET' && url.pathname === '/enterprise/me/export') {
        ...
        return json({ok:true,generatedAt:new Date().toISOString(),formatVersion:'1.0',account:{uid:user.sub,email:user.email||null,name:user.name||null},enterprise:{memberships:memberships.results||[],cohorts:cohorts.results||[],credentials:credentials.results||[],programCompletions:programCompletions.results||[],diagnostics:diagnostics.results||[],competencyScores:scores.results||[],readinessSnapshots:readiness.results||[],roleLabRuns:labs.results||[],assessmentAttempts:assessments.results||[]},note:'Assessment answer content and server-side answer keys are intentionally excluded from this export.'},200,env);
      }

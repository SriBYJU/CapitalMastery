(() => {
  ...
  function home(){
    ...
    render(`
      ...
      <section class="section section-white"><div class="container"><div class="section-head"><div><div class="eyebrow">TWO WAYS TO USE CAPITAL MASTERY</div><h2>Learn independently—or arrive prepared for your firm.</h2></div><p>The public learning platform stays open to individual learners. Employer workspaces add cohorts, firm-specific preparation and readiness analytics on top.</p></div><div class="grid grid-2"><article class="card"><div class="eyebrow">FOR INDIVIDUAL LEARNERS</div><h3>Build finance skills on your own.</h3><p>Choose any career pathway, build from foundations into applied work, complete simulations and earn verified credentials.</p><a class="btn btn-primary" href="#/careers">Explore Careers →</a></article><article class="card"><div class="eyebrow">FOR FINANCE FIRMS</div><h3>Prepare talent before Day 1.</h3><p>Create cohorts, assign role-specific readiness, add a Firm Layer, and see competency evidence instead of just course completion.</p><a class="btn btn-outline" href="#/employers">Capital Mastery for Employers →</a></article></div></div></section>
  ...
  function verifyPage(token){ const cred=state.credentials.find(x=>x.credentialId===decodeURIComponent(token)); const c=cred?careerById(cred.careerId):null; render(`<section class="section"><div class="container" style="max-width:850px">${cred&&c?`<div class="credential-summary"><span class="verify-status preview">QA PREVIEW RECORD</span><div class="eyebrow" style="margin-top:20px">CAPITAL MASTERY CREDENTIAL</div><h1>${esc(credentialInfo(c,cred.type).name)}</h1><p>Issued to <strong>${esc(state.profile.name)}</strong></p><div class="grid grid-2"><div class="card"><strong>Credential ID</strong><p>${esc(cred.credentialId)}</p></div><div class="card"><strong>Issued</strong><p>${formatDate(cred.issuedAt)}</p></div></div><div class="divider"></div><h3>Capital Mastery Standard</h3><p>100% required pathway completion · 80%+ every required knowledge assessment · 80%+ practical simulation for Career Certificates · 80%+ final examination for Career Certificates.</p><div class="feedback-box"><strong>Local QA preview:</strong> This browser-only preview is not an authoritative credential. Live accounts use the server-issued D1 verification record.</div></div>`:`<div class="card"><h1>Credential not found</h1><p>No local QA preview credential was found. Signed-in live credentials are verified from the authoritative D1 record.</p><a class="btn btn-primary" href="#/credentials">View Credentials</a></div>`}</div></section>`,'credentials'); }

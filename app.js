(() => {
  ...
  function footer(){
    return `<footer class="footer"><div class="container"><div class="footer-grid">
      ...
      <div><h4>Learn</h4>${link('careers','Careers')}${link('passport','My Learning')}${link('credentials','Credentials')}</div>
      <div><h4>About</h4>${link('about','About Capital Mastery')}${link('methodology','Research Methodology')}${link('credential-policy','Credential Policy')}</div>
  ...
  function home(){
    ...
    render(`
      <section class="hero"><div class="container hero-grid"><div>
        <div class="eyebrow">45+ FREE FINANCE CAREER CREDENTIALS</div>
        ...
        <div class="hero-path"><small>CAPITAL MASTERY STANDARD</small><h3>Completion is not enough.</h3><p class="small" style="color:#cbd5e2;margin:8px 0 0">Every required assessment, practical simulation and final exam must meet the 80% standard.</p></div>
      </div></div></section>
      <section class="stats-strip"><div class="container stats-grid">
        <div class="stat"><strong>${stats.marketingCredentials}</strong><span>Free credentials</span></div>
        ...
        <div class="stat"><strong>Real Data</strong><span>SEC · BLS · O*NET · Federal Reserve</span></div>
      </div></section>
  ...
  function careersPage(query){
    ...
    render(`<section class="page-hero"><div class="container"><div class="eyebrow">CAREER DIRECTORY</div><h1>Find your place in finance.</h1><p>Career field → exact entry role → required skills → realistic job simulation.</p></div></section>
  ...
  function technicalContent(c){
    return `<section class="lesson-section"><h2>The technical core</h2><p>Short learning units focus on one concept at a time: understand it, see it used, then apply it.</p><div class="concept-list">${c.concepts.map((id,i)=>conceptBlock(id,i+1)).join('')}</div></section>
  ...
  function conceptBlock(id,i){ const x=(DATA.concepts||{})[id]||{name:id.replace(/-/g,' ').replace(/\b\w/g,m=>m.toUpperCase()),explain:'Understand the concept, the assumptions behind it and where it appears in professional work.',work:'Apply the concept to a realistic finance decision.',example:'A professional answer connects the calculation to the decision.'}; return `<article class="concept-block"><div class="concept-index">${String(i).padStart(2,'0')}</div><div><h3>${esc(x.name)}</h3><p>${esc(x.explain)}</p><div class="concept-application"><strong>At the desk:</strong> ${esc(x.work)}</div><div class="concept-example"><strong>Example:</strong> ${esc(x.example)}</div></div></article>`; }
  function toolkitContent(c){
    return `<section class="lesson-section"><h2>Professional Toolkit</h2><p>This is where the pathway shifts from “know” to “do.” Each lab is tied to a work product you can expect in the role.</p><div class="grid grid-2">${c.toolkit.map((x,i)=>`<article class="lab-card"><div class="lab-num">LAB ${i+1}</div><h3>${esc(x)}</h3><p>${esc(toolkitWhy(c,x))}</p><div class="mini-work"><strong>Practice:</strong> ${esc(c.applied[i%c.applied.length])}</div></article>`).join('')}</div></section>
  ...
  function toolkitWhy(c,x){ return `Train ${x.toLowerCase()} with the same logic, source discipline and review mindset expected from a junior ${c.role}.`; }
  ...
  function appliedContent(c){
    return `<section class="lesson-section"><h2>Applied work before the full simulation</h2><p>You now complete realistic smaller assignments. Save drafts locally, revise after review, then take the Part 4 assessment.</p>${c.applied.map((x,i)=>`<article class="applied-assignment"><div class="assignment-head"><span>Assignment ${i+1}</span><strong>${esc(x)}</strong></div><label for="ap-${i}">Your work</label><textarea id="ap-${i}" data-applied="${i}" data-career="${c.id}" placeholder="Write your analysis, calculations, assumptions and recommendation here...">${esc(getCareerState(c.id).applied[i]||'')}</textarea><div class="manager-note"><strong>Manager review lens:</strong> accuracy · source quality · logic · risks · communication.</div></article>`).join('')}</section>
      <section class="lesson-section"><div class="feedback-box"><strong>Review loop:</strong> submit → manager comments → revise → final. Capital Mastery measures first-pass quality and response to feedback, not just whether you eventually reached the answer.</div></section>${sourceDrawer(c,'applied work')}`;
  }
  ...
  function quizPage(c,n,final=false){
    if(!final && !canAccessPart(c,n)) return nav(`career/${c.id}`);
    if(!final && !qaMode() && !getCareerState(c.id).learningComplete.includes(n)){ toast('Complete the required learning in this part before taking its assessment.','warn'); return nav(`learn/${c.id}/${n}`); }
    ...
    render(`<section class="quiz-shell"><div class="quiz-wrap"><div class="quiz-head"><div><div class="eyebrow">${final?'FINAL EXAMINATION':n===5?'JOB SIMULATION KNOWLEDGE CHECK':`PART ${n} ASSESSMENT`}</div><h1>${final?`${c.title} Final Examination`:PARTS[n-1].name}</h1><p>${q.length} questions · 80% required · ${final?'16/20 to pass':'8/10 to pass'}</p></div><a class="btn btn-outline" href="#/career/${c.id}">Exit</a></div><form id="quiz-form">${q.map((x,i)=>`<fieldset class="question"><legend><span>${i+1}</span>${esc(x.prompt)}</legend>${x.options.map((o,j)=>`<label class="option"><input type="radio" name="q${i}" value="${esc(o)}"><span>${String.fromCharCode(65+j)}</span><p>${esc(o)}</p></label>`).join('')}<div class="explanation" id="ex-${i}" hidden></div></fieldset>`).join('')}<button class="btn btn-primary btn-block" type="submit">Submit ${final?'Final Exam':'Assessment'}</button></form></div></section>`,'learning');
  ...
  function certificatePage(c,type){ const info=credentialInfo(c,type); if(!info.earned && !qaMode() && !eligible(c,type)){toast('This certificate is still locked.','warn');return nav(`career/${c.id}`);} const isCareer=type==='career', cls=type==='foundations'?'simple':type==='applied'?'applied':''; const description=isCareer?'for demonstrating mastery across required learning, technical assessments, applied work, a graded job simulation, and the final examination under the Capital Mastery Standard.':type==='applied'?'for successfully completing the applied learning, professional toolkit and required assessments.':'for successfully completing the career foundations and technical academy requirements.';
  ...
  function renderPseudoQr(el,text){
    // Deterministic visual verification mark for pre-Firebase build. Live QR will be generated server-side after Firebase.
  ...
  function linkedinPost(c,type,info,style){ const skills=[...c.vocab.slice(0,4),...c.deliverables.slice(0,3)]; if(style==='short')return `I earned the Capital Mastery ${info.name}. I completed required learning, assessments and applied work under an 80% mastery standard. #Finance #CapitalMastery`; if(style==='detailed')return `I'm excited to have earned the Capital Mastery ${info.name}.\n\nThe pathway covered ${skills.join(', ')} and required demonstrated mastery across learning, applied work${type==='career'?', a graded job simulation and a 20-question final examination':''}. Capital Mastery requires 80% or higher on each required assessment rather than awarding a participation certificate.\n\nCredential: ${location.origin}${location.pathname}#/verify/${encodeURIComponent(info.id)}\n\n#Finance #FinancialCareers #CapitalMastery`; return `I'm excited to have earned the Capital Mastery ${info.name}. Through the pathway, I developed and applied skills in ${skills.slice(0,5).join(', ')}${type==='career'?' and completed a graded job simulation plus final assessment':''}.\n\nCredential: ${location.origin}${location.pathname}#/verify/${encodeURIComponent(info.id)}\n\n#Finance #CapitalMastery`; }
  ...
  function verifyPage(token){ const cred=state.credentials.find(x=>x.credentialId===decodeURIComponent(token)); const c=cred?careerById(cred.careerId):null; render(`<section class="section"><div class="container" style="max-width:850px">${cred&&c?`<div class="credential-summary"><span class="verify-status preview">QA PREVIEW RECORD</span><div class="eyebrow" style="margin-top:20px">CAPITAL MASTERY CREDENTIAL</div><h1>${esc(credentialInfo(c,cred.type).name)}</h1><p>Issued to <strong>${esc(state.profile.name)}</strong></p><div class="grid grid-2"><div class="card"><strong>Credential ID</strong><p>${esc(cred.credentialId)}</p></div><div class="card"><strong>Issued</strong><p>${formatDate(cred.issuedAt)}</p></div></div><div class="divider"></div><h3>Capital Mastery Standard</h3><p>100% required pathway completion · 80%+ every required knowledge assessment · 80%+ practical simulation for Career Certificates · 80%+ final examination for Career Certificates.</p><div class="feedback-box"><strong>Pre-Firebase build:</strong> This is a local QA credential record. Live public verification becomes authoritative only after Firebase secure issuance is connected.</div></div>`:`<div class="card"><h1>Credential not found</h1><p>This pre-Firebase build can only verify credentials issued in this browser's local QA state.</p><a class="btn btn-primary" href="#/credentials">View Credentials</a></div>`}</div></section>`,'credentials'); }

  function loginPage(){ render(`<section class="section"><div class="container" style="max-width:650px"><div class="card"><div class="eyebrow">ACCOUNT SETUP</div><h1 class="serif" style="font-size:3rem;color:var(--navy)">Firebase connection pending.</h1><p>The complete product UI and local QA state are built first. Google sign-in, email/password, Firestore sync and secure admin claims will be connected when the Firebase project is created.</p><div class="feedback-box"><strong>Nothing else is blocked.</strong> You can test careers, quizzes, simulations, certificates, sharing, admin QA and screenshots locally without Firebase.</div><a class="btn btn-primary" href="#/careers">Continue in QA learner mode →</a></div></div></section>`); }

  function policyPage(kind){ const content={privacy:['Privacy','Capital Mastery will collect only the account, learning-progress, assessment and credential data necessary to provide the service. Firebase-backed production storage will be governed by security rules and a published data-retention policy before live accounts launch.'],terms:['Terms','Capital Mastery is an educational platform. Users may not misrepresent preview/test credentials as live verified credentials or interfere with assessment integrity. Full production terms will be finalized before Firebase account launch.'],disclaimer:['Educational Disclaimer','Capital Mastery provides education and simulated professional practice. It does not provide investment, legal, tax or employment advice; it does not guarantee admission, employment, compensation or professional licensing.'],credential:['Credential Policy','Capital Mastery certificates represent completion and demonstrated mastery of Capital Mastery educational requirements. They are not professional licenses or third-party accredited professional certifications unless explicitly stated. Final Career Certificates require 100% pathway completion and 80%+ on every required knowledge assessment, practical simulation and final examination.']}[kind]; render(`<section class="page-hero"><div class="container"><div class="eyebrow">POLICY</div><h1>${content[0]}</h1></div></section><section class="section-tight"><div class="container" style="max-width:850px"><div class="card"><p>${content[1]}</p><h3>Independent platform</h3><p>References to employers, universities, professional bodies or public agencies identify research sources only and do not imply affiliation, sponsorship or endorsement.</p><h3>Production status</h3><p>This build intentionally leaves account authentication and authoritative server-side credential issuance for the Firebase connection step.</p></div></div></section>`,'about'); }

  function adminPage(){
    const preview=qaMode();
    render(`<section class="page-hero"><div class="container"><div class="eyebrow">ADMIN / QA</div><h1>Capital Mastery Release Lab</h1><p>Full feature preview without hard-coding an admin password. Firebase will later protect this area with a server-verified admin claim.</p></div></section><section class="section-tight"><div class="container"><div class="admin-grid"><div class="admin-card"><h3>QA Preview Mode</h3><p>${preview?'Enabled':'Disabled'}</p><button class="btn btn-primary btn-sm" onclick="CM.toggleQa()">${preview?'Disable':'Enable'} QA Mode</button></div><div class="admin-card"><h3>Credential Lab</h3><p>Preview all three certificate levels without creating a live verified record.</p><div class="admin-actions"><a class="btn btn-soft btn-sm" href="#/certificate/investment-banking/foundations">Foundations</a><a class="btn btn-soft btn-sm" href="#/certificate/investment-banking/applied">Applied</a><a class="btn btn-gold btn-sm" href="#/certificate/investment-banking/career">Career</a></div></div><div class="admin-card"><h3>Boundary Tests</h3><p>Set IB demo scores to 79, 80 or 100 and verify pass/fail behavior.</p><div class="admin-actions"><button class="btn btn-soft btn-sm" onclick="CM.qaScores(79)">79%</button><button class="btn btn-soft btn-sm" onclick="CM.qaScores(80)">80%</button><button class="btn btn-soft btn-sm" onclick="CM.qaScores(100)">100%</button></div></div><div class="admin-card"><h3>Progress States</h3><p>Jump the flagship pathway to a testing state.</p><div class="admin-actions">${[0,20,40,60,80,100].map(p=>`<button class="btn btn-soft btn-sm" onclick="CM.qaProgress(${p})">${p}%</button>`).join('')}</div></div><div class="admin-card"><h3>Simulation Lab</h3><p>Jump directly to the full practical simulation.</p><a class="btn btn-primary btn-sm" href="#/simulation/investment-banking">Open Project Northstar</a></div><div class="admin-card"><h3>Reset QA State</h3><p>Clear local progress and credentials. Firebase data is not involved.</p><button class="btn btn-danger btn-sm" onclick="CM.resetState()">Reset Local State</button></div></div><div class="section-head" style="margin-top:50px"><div><div class="eyebrow">CURRENT LOCAL STATE</div><h2>Debug snapshot</h2></div></div><pre class="admin-state">${esc(JSON.stringify(state,null,2))}</pre></div></section>`,'learning'); }
  ...
  function bindGlobal(){ document.querySelectorAll('textarea[data-applied]').forEach(t=>t.addEventListener('input',()=>{const cs=getCareerState(t.dataset.career);cs.applied[t.dataset.applied]=t.value;saveState();})); }

  function renderRoute(){
    const {parts,query}=routeParts(); const [root,a,b]=parts; document.title='Capital Mastery | 45+ Free Finance Credentials | Made by Shriyan Avadhanula';
    try{
      if(!root) return home();

(() => {
  ...
  function certificatePage(c,type){ const info=credentialInfo(c,type); if(!info.earned && !qaMode() && !eligible(c,type)){toast('This certificate is still locked.','warn');return nav(`career/${c.id}`);} const isCareer=type==='career', cls=type==='foundations'?'simple':type==='applied'?'applied':''; const description=isCareer?'for demonstrating mastery across required learning, technical assessments, applied work, a graded job simulation, and the Professional Readiness Final under the Capital Mastery Standard.':type==='applied'?'for successfully completing the applied learning, professional toolkit and required assessments.':'for successfully completing the career foundations and technical academy requirements.';
